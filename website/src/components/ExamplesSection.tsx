            ...
            <div key={index}>
              <GlassCard glowColor="none" className="h-full group p-6 sm:p-8">
                <div className="flex items-center gap-4 mb-6">
                  <div className="p-3 rounded-full bg-white text-black group-hover:bg-gray-200 transition-colors">
                    <example.icon className="h-5 w-5" />
                  </div>
                  <div>
                    <span className="text-[8px] font-black px-3 py-1 rounded-full bg-white/10 text-white uppercase tracking-widest mb-2 inline-block">
                      {example.category}
                    </span>
                ...
                <div className="space-y-4 pl-2 border-l border-white/10 ml-5">
                  {example.examples.map((text, idx) => (
                    <div key={idx} className="relative group/bubble">
                      <div className="absolute -left-[23px] top-3 w-2.5 h-2.5 rounded-full bg-white/10 border border-white/20 group-hover/bubble:bg-white transition-colors"></div>
